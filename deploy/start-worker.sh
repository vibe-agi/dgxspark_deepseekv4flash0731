...
validate_roce_interface() {
    ...
    if [ "$local_ip" != "$expected_ip" ]; then
        echo "❌ Worker RoCE IP 不匹配: $intf 期望 $expected_ip，实际 ${local_ip:-未配置}"
        exit 1
    fi
    if [ "$mtu" != "9000" ]; then
        echo "❌ Worker RoCE MTU 不匹配: $intf 期望 9000，实际 $mtu"
        exit 1
    fi
    echo "✅ RoCE 网卡: $intf (IP=$local_ip, MTU=$mtu)"
}

validate_roce_interface "$NCCL_INTF" "$WORKER_IP"
validate_roce_interface "$NCCL_INTF_SECONDARY" "$WORKER_IP_SECONDARY"
